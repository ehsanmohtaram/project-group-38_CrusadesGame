    ...
    public void signUpInfo(Pane pane) {
        ...
        signUp.setOnMouseClicked(mouseEvent -> {
            ...
            if (checkForErrors(userError, nickNameError, emailError, passwordError, securityError)) {
                ...
                User.addUser(userName.getText(), nickName.getText(), DigestUtils.sha256Hex(((TextField) passwordFiled.getChildren().get(0)).getText()), email.getText(), sloganText.getText(), choiceBox.getValue(),securityAnswer.getText(), SignUpMenu.class.getResource("/images/avatars/1.jpg").toString());
            }
        });
    }
