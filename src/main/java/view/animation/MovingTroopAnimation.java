...
public class MovingTroopAnimation extends Transition {
    ...
    protected void interpolate(double v) {
        ...
        if (isRight) {
            if (isFight)
                rectangle.setFill(new ImagePattern(new Image(LoginMenu.class.getResource("/images/animation/unit/" + folderName + "/right/" + (8 + i) + ".png").toExternalForm())));
            else
            ...
            if (isFight)
                rectangle.setFill(new ImagePattern(new Image(LoginMenu.class.getResource("/images/animation/unit/" + folderName + "/left/" + (8 + i) + ".png").toExternalForm())));
            else
